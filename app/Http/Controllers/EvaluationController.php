...
class EvaluationController extends Controller
{
    ...
    public function index(Examen $examen, Request $request)
    {
        ...
        $evaluations = ExamenEvaluation::with([
            ...
            'enchainement:id,name,order,diviseur',
        ...
        return response()->json([
            ...
            'students' => $students,
            'total_points_possibles' => $totalPointsPossibles,
        ], 200);
    }
    ...
    public function store(StoreEvaluationReq $request, Examen $examen, ExamenCandidat $candidat)
    {
        try {
            ...
            if ($examen->organisateur_id !== $activeId) {
                return response()->json(['message' => 'Vous n\'êtes pas l\'organisateur de cet examen'], 403);
            }
            if ($candidat->examen_id != $examen->id) {
                return response()->json(['success' => false, 'message' => 'Vous ne pouvez pas noter cet étudiant'], 403);
            }
            if ($candidat->status !== ExamenCandidat::STATUS_REGISTERED) {
                return response()->json(['success' => false, 'message' => 'Statut candidat invalide'], 403);
            }

            // 2. RÉCUPÉRATION DES ENCHAÎNEMENTS
            ...
            foreach ($validated['notes'] as $note) {
                ...
                if ($note['score'] > $enchainements[$note['enchainement_id']]->diviseur) {
                    return response()->json(['success' => false, 'message' => 'Score supérieur au maximum autorisé'], 400);
                }
            }

            // 4. ENREGISTREMENT DES NOTES INDIVIDUELLES
            foreach ($validated['notes'] as $note) {
            ...
            // 5. CALCUL DU SCORE TOTAL ET DU SEUIL DYNAMIQUE
            $totalScore = collect($validated['notes'])->sum(fn($n) => (float) $n['score']);

            $totalPointsPossibles = $enchainements->sum('diviseur');
            $seuilAdmission = $totalPointsPossibles / 2;

            // --- NOUVELLE LOGIQUE : VÉRIFICATION DES NOTES ÉLIMINATOIRES ---
            $aUneNoteEliminatoire = false;

            // On ne vérifie l'élimination que pour la Ligue (Ceinture Noire)
            if ($examen->organisateur_type === 'Ligue') {
                foreach ($validated['notes'] as $note) {
                    $maxDeCetteMatiere = $enchainements[$note['enchainement_id']]->diviseur;
                    $moyenneDeCetteMatiere = $maxDeCetteMatiere / 2;

                    // Si la note est strictement inférieure à la moyenne de la matière
                    if ((float)$note['score'] < $moyenneDeCetteMatiere) {
            ...
            // Décision finale : Doit avoir la moyenne générale ET ne pas avoir de note éliminatoire
            $isAdmis = ($totalScore >= $seuilAdmission) && !$aUneNoteEliminatoire;

            $passage = $isAdmis ? 'Admis' : 'Ajourné';

            // Log pour le debug en production
            if ($aUneNoteEliminatoire) {
                Log::info('Candidat éliminé par note inférieure à 10', ['student_id' => $candidat->student_id]);
            }
            // 6. MISE À JOUR DU RÉSULTAT FINAL
            ...
            if ($passage == 'Admis') {
                ...
                $studentGrade = StudentGrade::updateOrCreate([
                    'student_id'       => $candidat->student_id,
                    'current_grade_id' => $examen->next_grade_id,
                ], [
                    'club_id'          => $studentClubId,
                    'awarded_at'       => now(),
            ...
            return response()->json([
                'success' => true,
                'message' => 'Votre note a bien été enregistrée : ' . $passage,
                'result' => $result,
                'studentGrade' => $studentGrade,
            ], 201);
        } catch (QueryException $e) {
            $errcode = $e->getCode();
            Log::error('Erreur SQL Notation', ['code' => $errcode, 'message' => $e->getMessage()]);

            if ($errcode == '23000') {
                return response()->json(['success' => false, 'message' => 'Données en double ou conflit de grade'], 422);
            }
            return response()->json(['success' => false, 'message' => 'Une erreur SQL est survenue'], 500);
        } catch (\Exception $e) {
            Log::error('Erreur Générale Notation', ['message' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => 'Une erreur est survenue'], 500);
        }
    }
    //update notes
    public function update(StoreEvaluationReq $request, Examen $examen, string $studentId)
    {

        return DB::transaction(function () use ($request, $examen, $studentId) {
            try {
                $validated = $request->validated();
                $user = auth()->user();
                $activeId = $request->attributes->get('organisateur_id');
                ...
                $candidat = ExamenCandidat::where('examen_id', $examen->id)
